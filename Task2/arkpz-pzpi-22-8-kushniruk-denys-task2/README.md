...
API відповідає за обробку логів
...
- **Request Body**:
  - `user_id` (string): ID користувача, який здійснив дію.
  - `room_id` (string): ID кімнати, в якій відбулася дія.
  - `action` (string): Опис дії.
  - `timestamp` (string, date-time): Час виконання дії.
- **Responses**:
  - `200`: Лог успішно створено.
